...
public abstract class ShellSAIPackageInstaller extends SAIPackageInstaller {
        ...
        try (ApkSource apkSource = aApkSource) {
            ...
            if (!getShell().isAvailable()) {
                dispatchCurrentSessionUpdate(InstallationStatus.INSTALLATION_FAILED,
                                             getContext().getString(R.string.installer_error_shell,
                                                                    getInstallerName(),
                                                                    getShellUnavailableMessage()));
                installationCompleted();
                return;
            }
            ...
            while (apkSource.nextApk()) {
                ...
                ensureCommandSucceeded(getShell().exec(new Shell.Command("pm",
                                                                         "install-write",
                                                                         "-S",
            ...
            Shell.Result installationResult = getShell().exec(new Shell.Command("pm",
                                                                                "install-commit",
                                                                                String.valueOf(sessionId)));
    ...
    private String getSessionInfo(ApkSource apkSource) {
        ...
        return String.format("%s: %s %s | %s | Android %s | Using %s ApkSource implementation | SAI %s",
                             getContext().getString(R.string.installer_device),
                             Build.BRAND,
                             Build.MODEL,
                             MiuiUtils.isMiui() ? "MIUI" : "Not MIUI",
                             Build.VERSION.RELEASE,
                             apkSource.getClass().getSimpleName(),
                             saiVersion);
    }
    ...
    private int createSession() throws RuntimeException {
        ...
        if (customInstallCreateCommand != null) {
            ...
            commandsToAttempt.add(new Shell.Command("pm",
                                                    "install-create",
                                                    "-r",
                                                    "--install-location",
                                                    installLocation,
                                                    "-i",
                                                    getShell().makeLiteral(BuildConfig.APPLICATION_ID)));
            commandsToAttempt.add(new Shell.Command("pm",
                                                    "install-create",
                                                    "-r",
                                                    "-i",
