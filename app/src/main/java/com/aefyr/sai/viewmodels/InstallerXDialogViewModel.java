...
public class InstallerXDialogViewModel extends ViewModel {
    ...
    public void enqueueInstallation() {
        ...
        for (UriMessResolutionResult resolutionResult : mResolutionResults) {
            ...
            if (SourceType.ZIP.equals(resolutionResult.sourceType())) {
                apkSourceBuilder = new ApkSourceBuilder(mContext)
                        .fromZipContentUri(resolutionResult.uris().get(0));

            } else if (SourceType.APK_FILES.equals(resolutionResult.sourceType())) {
                apkSourceBuilder = new ApkSourceBuilder(mContext)
    ...
    private static class LoadMetaTaskResult {
        ...
        private LoadMetaTaskResult(@Nullable SplitApkSourceMeta meta, @Nullable Set<String> splitsToSelect,
                                   @NonNull List<UriMessResolutionResult> resolutionResults) {
            this.meta = meta;
    ...
    private class LoadMetaTask extends SimpleAsyncTask<LoadMetaTaskInput, LoadMetaTaskResult> {
        ...
        protected LoadMetaTaskResult doWork(LoadMetaTaskInput input) {
            ...
            DefaultSplitApkSourceMetaResolver metaResolver = new DefaultSplitApkSourceMetaResolver(mContext,
                                                                                                   new DefaultAppMetaExtractor(
                                                                                                           mContext));
            ...
            if (mResolutionResults.isEmpty()) {
                ...
                if (uriMessResolutionResult.isSuccessful()) {
                    ...
                    if (error.doesTryingToInstallNonethelessMakeSense()) {
                        mWarning = new Warning(mContext.getString(R.string.installerx_dialog_resolution_error_non_critical,
                                                                  uriMessResolutionResult.error().message()), true);
                    } else {
                        mWarning = new Warning(mContext.getString(R.string.installerx_dialog_resolution_error_critical,
                                                                  uriMessResolutionResult.error().message()), false);
                    }
                    mState.setValue(State.WARNING);
                }
            } else {
